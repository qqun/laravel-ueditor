...
abstract class Upload
{
    ...
    public function __construct(array $config, $request)
    {
        ...
        $stateMap = [
            ...
            "ERROR_UNKNOWN_MODE" => trans("UEditor::upload.ERROR_UNKNOWN_MODE"),   //qiniu或本地//

            "INVALID_URL" => trans("UEditor::upload.INVALID_URL"),
            "INVALID_IP" => trans("UEditor::upload.INVALID_IP"),
        ];
...
//             "url" => $this->fullName,
        ...
        if (config('Ueditor.core.mode') == self::QINIU_MODEL) {
            $fileInfo['url'] = $this->fullName;
        } else if (config('Ueditor.core.mode') == self::UPYUN_MODEL) {
            $fileInfo['url'] = $this->fullName;
        } else {
            $fileInfo['url'] = config('Ueditor.core.baseurl') . $this->fullName;
        }
        return $fileInfo;
    }
